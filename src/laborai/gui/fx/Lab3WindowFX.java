...
import Lab3_Jecka_Ernestas.Kasininkas;
import Lab3_Jecka_Ernestas.KasininkuGeneravimas;
import java.io.File;
...
public class Lab3WindowFX extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private MapADTx<String, Kasininkas> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private String key;
    private HashType ht = HashType.DIVISION;
    private final KasininkuGeneravimas kasGeneravimas = new KasininkuGeneravimas();
    private Kasininkas kasIterpimui = new Kasininkas();
    ...
    private void initComponents() {
        ...
        paneButtons = new PanelsFX(
                new String[]{
                    ...
                    MESSAGES.getString("button9")}, 1, 9);
    ...
    public void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Kasininkas[] kasArray = kasGeneravimas.generuotiIrPateiktiKasininku(sizeOfGenSet, sizeOfInitialSubSet);
            kasIterpimui = kasGeneravimas.imtiKasininka();
            for (Kasininkas a : kasArray) {
                map.put(
                        kasGeneravimas.gautiIsBazesKasininkoId(), //raktas
                        a);
            }
    ...
    public void takeFromMap() {
        readMapParameters();
        kasIterpimui = map.get(key);
        KsFX.ounArgs(taEvents, MESSAGES.getString("msg9"), kasIterpimui);
    }
    
    public void replace(){
        if (kasIterpimui == null && key == null) {
            KsFX.ounerr(taEvents, MESSAGES.getString("Nepasirinktas kasininkas arba raktas"));
        }
        readMapParameters();
        Kasininkas a = (Kasininkas) ((MapKTU) map).replace(this.key, this.kasIterpimui);
        if (a != null) {
            table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
            String[][] modelList = map.getModelList(paneParam1.getTfOfTable().get(5).getText());
            table.setItems(FXCollections.observableArrayList(modelList));
            updateHashtableParameters(true);
            KsFX.ounArgs(taEvents, "Pakeistas elementas: %s", a);
        } else{
            KsFX.ounArgs(taEvents, "Su tokiu raktu reikšmės nebuvo, keitimas nepavyko");
        }
    }
    ...
    public void putIfAbsent(){
        ...
        readMapParameters();
        Kasininkas a = (Kasininkas) ((MapKTU) map).putIfAbsent(this.key, this.kasIterpimui);
        if (a == null) {
            table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
            String[][] modelList = map.getModelList(paneParam1.getTfOfTable().get(5).getText());
            table.setItems(FXCollections.observableArrayList(modelList));
            updateHashtableParameters(true);
            KsFX.ounArgs(taEvents, "Įterpta Reikšmė: %s", kasIterpimui);
        }
            KsFX.ounArgs(taEvents, MESSAGES.getString("msg11"), a);
    }
    
    public void containsValue() {
        if (kasIterpimui == null) {
            KsFX.ounerr(taEvents, MESSAGES.getString("Nepasirinktas Norimas kasininkas"));
        }
        boolean a = ((MapKTU) map).containsValue(this.kasIterpimui);
        KsFX.ounArgs(taEvents, MESSAGES.getString("msg10"), a);
    }
    
    public void averageChainSize() {
        int a = ((MapKTU) map).averageChainSize();
        KsFX.ounArgs(taEvents, MESSAGES.getString("msg7"), a);
    }
    
    public void numberOfEmties() {
        int a = ((MapKTU) map).numberOfEmties();
        KsFX.ounArgs(taEvents, MESSAGES.getString("msg8"), a);
    }
    
    public void mapAdd() {
        Kasininkas a = kasGeneravimas.imtiKasininka();
        map.put(
                kasGeneravimas.gautiIsBazesKasininkoId(), // Raktas
                a);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
    ...
    private void readMapParameters() {
        ...
        List<TextField> tfs2 = paneParam3.getTfOfTable();
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Kasininkas(), initialCapacity, loadFactor, ht);
                break;
            case 1:
                 map = new MapKTUOAx<>(new String(), new Kasininkas(), initialCapacity, loadFactor, ht);
                 break;
            default:
                IntStream.of(1, 3).forEach(p -> paneButtons.getButtons().get(p).setDisable(true));
