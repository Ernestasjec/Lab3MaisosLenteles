...
public class MapKTU<K, V> implements MapADTp<K, V> {
    ...
    /**
     * pakeicia reiksme, jei elementas jau egzistuoja, su tuo raktu.
     * @param key
     * @param value
     * @return 
     */
    public V replace(K key, V value){
        if (key == null || value == null) {
            throw new IllegalArgumentException("Key or value is null in getInChain(K key, V value)");
        }
        int index = hash(key, ht);
        Node<K, V> n = getInChain(key, table[index]);
        if (n == null) {
            return null;
        }
        V val = n.value;
    ...
    /**
     * ideda reiksme, jei nera jau susieta reiksme su pateiktu raktu
     * @param key
     * @param value
     * @return null jei istato reiksme
     */
    public V putIfAbsent(K key, V value){
        if (key == null || value == null) {
            throw new IllegalArgumentException("Key or value is null in putIfAbsent(K key, V value)");
        }
        int index = this.hash(key, ht);
        Node<K, V> n = getInChain(key, table[index]);
        if (n == null) {
            this.put(key, value);
            return null;
    ...
    /**
     * Tikrina ar yra ieškoma vertė
     * @param value ieškoma vertė
     * @return true jei yra false, jei nėra
     */
    public boolean containsValue(V value){
        if (value == null) {
            throw new NullPointerException("in method containsValue value = null");
        }
            for (int i = 0; i < table.length; i++) {
    ...
    /**
     * skaičiuoja vidutinį grandinėlės ilgį
     * @return 
     */
   public int averageChainSize()
   {
    if (table == null) {
        ...
        return (int) Math.round(size * 1.0 / this.chainsCounter);
   }
   /**
    * Skaičiuoja tuščių elementų skaičių
    * @return tuščių elementų skaičius
    */
    public int numberOfEmties(){
        return table.length - this.chainsCounter;
    }
    @Override
